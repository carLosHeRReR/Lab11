...
export class TiendasComponent implements OnInit{
  ...
  markers: MarkerProperties[] = [
    ...
    { position: { lat: -34.588889, lng: -58.430833 }, 
       label: { color: 'black', text: 'Tienda N°4', fontSize: '20px', fontWeight: 'bold'  },
       title: 'ciudad',
       info: 'Palermo'
        },
    { position: { lat: 35.8939, lng: 137.4758 }, 
        label: { color: 'black', text: 'Tienda N°5', fontSize: '20px', fontWeight: 'bold'  },
        title: 'ciudad',
        info: 'Cordillera Kiso  '
        }
  ];
  ...
  verTiendas( distrito: string){
    ...
    if(distrito == 'Cordillera Kiso'){
      this.verCordilleraKiso();
    }
  ...
  verPalermo(){
    const palermoLatLng = new google.maps.LatLng(-34.588889, -58.430833);
    this.map.setCenter(palermoLatLng);
    this.map.setZoom(17);
  }
  
  verCordilleraKiso(){
    const palermoLatLng = new google.maps.LatLng(35.8939, 137.4758);
    this.map.setCenter(palermoLatLng);
    this.map.setZoom(17);
  }
